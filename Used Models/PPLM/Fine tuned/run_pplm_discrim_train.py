...
class Discriminator(torch.nn.Module):
    ...
    def __init__(
        ...
        if pretrained_model.startswith("gpt2"):
            ...
            self.encoder = GPT2LMHeadModel.from_pretrained('trained_model/', local_files_only=True)
...
def get_idx2class(dataset_fp):
    ...
    with open(dataset_fp) as f:
        rows = json.load(f)
        for row in tqdm(rows, ascii=True):
            if row:
...
def get_generic_dataset(dataset_fp, tokenizer, device,
    ...
    with open(dataset_fp) as f:
        rows = json.load(f)
        for i, row in enumerate(tqdm(rows, ascii=True)):
            if row:
                label = row['label']
                text = row['text']
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_fp", type=str, default="upwork//PPLM//stances.json",
